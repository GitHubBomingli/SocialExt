...
@implementation SEAblumList

@end

@implementation SEPhotosObject

static SEPhotosObject *sharePhotoObject = nil;
+ (instancetype)sharePhotoObject {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharePhotoObject = [[self alloc] init];
    });
    return sharePhotoObject;
}

#pragma mark - 获取所有相册列表
- (NSArray<SEAblumList *> *)getPhotoAblumList {
    NSMutableArray<SEAblumList *> *photoAblumList = [NSMutableArray array];
    ...
    [smartAlbums enumerateObjectsUsingBlock:^(PHAssetCollection * _Nonnull collection, NSUInteger idx, BOOL *stop) {
        ...
        if (!([collection.localizedTitle isEqualToString:@"Recently Deleted"] ||
            ...
            if (assets.count > 0) {
                SEAblumList *ablum = [[SEAblumList alloc] init];
...
- (void)requestImageMaximumSizeForAsset:(PHAsset *)asset resizeMode:(PHImageRequestOptionsResizeMode)resizeMode completion:(void (^)(UIImage *))completion {
    PHImageRequestOptions *option = [[PHImageRequestOptions alloc] init];
    /**
     resizeMode：对请求的图像怎样缩放。有三种选择：None，默认加载方式；Fast，尽快地提供接近或稍微大于要求的尺寸；Exact，精准提供要求的尺寸。
     deliveryMode：图像质量。有三种值：Opportunistic，在速度与质量中均衡；HighQualityFormat，不管花费多长时间，提供高质量图像；FastFormat，以最快速度提供好的质量。
    ...
    option.networkAccessAllowed = YES;
    //param：targetSize 即你想要的图片尺寸，若想要原尺寸则可输入PHImageManagerMaximumSize
    [[PHCachingImageManager defaultManager] requestImageForAsset:asset targetSize:PHImageManagerMaximumSize contentMode:PHImageContentModeAspectFill options:option resultHandler:^(UIImage * _Nullable image, NSDictionary * _Nullable info) {
        completion(image);
    }];
}
